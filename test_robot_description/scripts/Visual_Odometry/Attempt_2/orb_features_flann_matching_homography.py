...
while True:
    ...
    if keypoints and descriptors is not None:
        ...
        if len(good_matches) >= 4 and prev_keypoints is not None and prev_descriptors is not None:
            src_pts = np.float32([keypoints[m.queryIdx].pt for m in good_matches]).reshape(-1, 1, 2) # source points -> current frame
            dst_pts = np.float32([prev_keypoints[m.trainIdx].pt for m in good_matches]).reshape(-1, 1, 2) # destination points -> previous frame
