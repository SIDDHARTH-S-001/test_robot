...
# Initialize initial homography matrix as an identity matrix
initial_H = np.eye(3)

def homography_residuals(H_flat, src_pts, dst_pts):
            ...
            if prev_frame is not None and prev_keypoints is not None:
                ...
                if len(good_matches) >= 4 and prev_keypoints is not None and prev_descriptors is not None:
                    ...
                    if len(src_pts) == len(dst_pts) >= 4:
                        # Optimize homography matrix using least squares
                        result = least_squares(homography_residuals, initial_H.flatten(), args=(src_pts, dst_pts), method='lm')

                        # Reshape the optimized parameters to obtain the homography matrix
                        H_flat = result.x
                        H = np.array([[H_flat[0], H_flat[1], H_flat[2]],
                        ...
                        theta_rad = round(-np.arctan2(-H[0, 1], H[0, 0]) * 360 / np.pi, 2)
