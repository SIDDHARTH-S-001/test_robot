...
class OccupancyGridMapping:
    ...
    def update_map_size(self):
        ...
        # Update map sizes in stored maps
        for i in range(len(self.maps)):
            self.maps[i] = (np.zeros((self.map_width, self.map_height), dtype=np.int8), self.maps[i][1])

    def publish_map(self):
        for map_data, (pose_x, pose_y) in self.maps:
            ...
            map_msg.info.origin.position.x = pose_x - self.map_origin_x
            map_msg.info.origin.position.y = pose_y - self.map_origin_y
